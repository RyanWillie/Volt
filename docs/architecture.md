...
electrical, net-class, parts, BOM sourcing, IO serialization, and assembly CPL option
...
subsystems (schematic model, PCB model outside the assembly CPL option boundary,
adapters, authoring) still throw raw `std::logic_error`, `std::invalid_argument`, or
`std::out_of_range` until their migration lands. Until then, catching `volt::KernelError`
alone does not cover every mutation-boundary failure.
