...
void bind_circuit(pybind11::module_ &module) {
    ...
    py::class_<PyPartLibrary>(module, "PartLibrarySnapshot")
        ...
        .def("electrical_model", &PyPartLibrary::electrical_model, py::arg("part_key"))
    ...
    module.def(
        "part_library_bundle_electrical_model",
        [](const py::bytes &bytes, const std::string &part_key) {
            return PyPartLibraryBundle{static_cast<std::string>(bytes)}.electrical_model(part_key);
        },
        py::arg("bytes"), py::arg("part_key"));
