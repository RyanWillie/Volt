...
[[nodiscard]] inline std::string pcb_svg_escape(std::string_view value) {
...
inline void write_pcb_svg_number(std::ostream &out, double value) {
...
[[nodiscard]] inline PcbSvgBounds bounds_from_outline(const Board &board) {
...
[[nodiscard]] inline double preview_width(const PcbSvgBounds &bounds) {
...
[[nodiscard]] inline std::string entity_ref_svg_id(EntityRef entity) {
    switch (entity.kind()) {
...
[[nodiscard]] inline std::string entity_ref_list(const Diagnostic &diagnostic) {
    auto result = std::string{};
...
[[nodiscard]] inline std::string severity_class(Severity severity) {
    switch (severity) {
    case Severity::Info:
        return "info";
...
[[nodiscard]] inline std::string footprint_ref_token(const FootprintRef &ref) {
    return ref.library() + ":" + ref.name();
}

[[nodiscard]] inline FootprintLibrary
preview_footprint_library(const Board &board, const FootprintLibrary &footprints) {
...
[[nodiscard]] inline const FootprintDefinition *
...
[[nodiscard]] inline PcbSvgBounds footprint_local_bounds(const FootprintDefinition &definition) {
...
inline void write_pcb_svg_outline(std::ostream &out, const Board &board) {
    if (!board.outline().has_value()) {
        return;
    }
...
inline void write_pcb_svg_features(std::ostream &out, const Board &board) {
...
inline void write_pad(std::ostream &out, const FootprintPad &pad, FootprintPadId pad_id,
...
inline void write_pad_overlays(std::ostream &out, const Board &board,
                               const std::vector<PadResolution> &resolutions,
                               PcbPlacementSvgOptions options) {
...
inline void write_pcb_placement_svg(std::ostream &out, const Board &board,
                                    const FootprintLibrary &footprints,
                                    PcbPlacementSvgOptions options = {}) {
...
[[nodiscard]] inline std::string write_pcb_placement_svg(const Board &board,
                                                         const FootprintLibrary &footprints,
                                                         PcbPlacementSvgOptions options = {}) {
