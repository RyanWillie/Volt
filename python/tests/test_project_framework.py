...
def test_project_result_writes_part_model_assets_and_placement_transforms(tmp_path):
    ...
    assert [artifact["path"] for artifact in manifest["artifacts"]] == [
        ...
        "assets/models/{}.glb".format(asset_hash),
    ...
    registry = json.loads(
        (output / "assets" / "part_models_3d.json").read_text(encoding="utf-8")
    )
    ...
    placements = json.loads(
        (output / "pcb" / "Main.volt.models3d.json").read_text(encoding="utf-8")
    )
...
def test_project_result_viewer_profile_reports_missing_part_model_assets(tmp_path):
    ...
    diagnostics = json.loads(
        (output / "diagnostics" / "diagnostics.json").read_text(encoding="utf-8")
    )
