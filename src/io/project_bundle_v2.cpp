...
open_verified(const detail::BundleSource &source, detail::CapturedEntry manifest_capture) {
    ...
    verify_report(storage->v2_artifacts[storage->v2_diagnostics].bytes, ArtifactKind::Diagnostics,
                  manifest.run);
    verify_report(storage->v2_artifacts[storage->v2_tests].bytes, ArtifactKind::ProjectTests,
                  manifest.run);
