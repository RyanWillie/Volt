...
class SchematicEndpoint {
  public:
    ...
    explicit SchematicEndpoint(Point position) : position_{position} {}

    /** Construct a sheet point tied to an existing concrete logical pin. */
    SchematicEndpoint(Point position, PinId pin) : position_{position}, pin_{pin} {}
...
class Schematic {
  public:
    ...
    add_sheet_port_for_endpoint(SheetId sheet, std::optional<NetId> net,
        ...
        return add_sheet_port(sheet, SheetPort{resolved_net, std::move(name), kind,
    ...
    [[nodiscard]] NetLabelId add_net_label_for_endpoint(
        ...
        return add_net_label(sheet,
                             NetLabel{resolved_net, endpoint.position(), orientation,
    ...
    [[nodiscard]] std::optional<NetId> infer_endpoint_net(const SchematicEndpoint &endpoint) const {
        if (endpoint.pin().has_value()) {
            ...
            if (!pin_net.has_value()) {
                throw std::invalid_argument{"Schematic endpoint " +
                                            pin_label(endpoint.pin().value()) +
        ...
        if (endpoint_net.has_value() && endpoint_net.value() != net) {
            if (endpoint.pin().has_value()) {
                throw std::invalid_argument{
                    ...
                    ": the pin belongs to " + net_label(endpoint_net.value()) + " instead of " +
                    net_label(net)};
            }
