...
pub fn export_step(board: &Board) -> String {
    ...
    writeln!(out, "FILE_NAME('{}','2026-04-17',('Volt EDA'),(''),'',' ','');", board.name).unwrap();
    ...
    // Application context
    let app_ctx = id; id += 1;
    writeln!(out, "#{}=APPLICATION_CONTEXT('automotive design');", app_ctx).unwrap();

    let app_proto = id; id += 1;
    writeln!(out, "#{}=APPLICATION_PROTOCOL_DEFINITION('','automotive_design',2010,#{});", app_proto, app_ctx).unwrap();

    // Product
    let product = id; id += 1;
    writeln!(out, "#{}=PRODUCT('{}','{}','',(#{}));", product, board.name, board.name, id).unwrap();

    let prod_ctx = id; id += 1;
    writeln!(out, "#{}=PRODUCT_CONTEXT('',#{}, 'mechanical');", prod_ctx, app_ctx).unwrap();

    let prod_def_form = id; id += 1;
    writeln!(out, "#{}=PRODUCT_DEFINITION_FORMATION('','',#{});", prod_def_form, product).unwrap();

    let prod_def_ctx = id; id += 1;
    writeln!(out, "#{}=PRODUCT_DEFINITION_CONTEXT('design',#{}, 'design');", prod_def_ctx, app_ctx).unwrap();

    let prod_def = id; id += 1;
    writeln!(out, "#{}=PRODUCT_DEFINITION('design','',#{},#{});", prod_def, prod_def_form, prod_def_ctx).unwrap();

    // Shape
    let shape_def = id; id += 1;
    let shape_rep = id; id += 1;
    writeln!(out, "#{}=PRODUCT_DEFINITION_SHAPE('','',#{});", shape_def, prod_def).unwrap();

    let shape_def_rep = id; id += 1;
    writeln!(out, "#{}=SHAPE_DEFINITION_REPRESENTATION(#{},#{});", shape_def_rep, shape_def, shape_rep).unwrap();
    ...
    let axis = id; id += 1;
    writeln!(out, "#{}=AXIS2_PLACEMENT_3D('',#{},#{},#{});", axis, origin, dir_z, dir_x).unwrap();
    ...
    for (x, y) in &vertices {
        let pt = id; id += 1;
        writeln!(out, "#{}=CARTESIAN_POINT('',({:.6},{:.6},{:.6}));", pt, x, y, thickness).unwrap();
        top_pts.push(pt);
    }
    ...
    let polyline_bottom = id; id += 1;
    let pts_list: String = bottom_pts.iter().map(|p| format!("#{}", p)).collect::<Vec<_>>().join(",");
    writeln!(out, "#{}=POLYLINE('bottom_outline',({}));", polyline_bottom, pts_list).unwrap();

    // Polyline for top outline
    let polyline_top = id; id += 1;
    let pts_list: String = top_pts.iter().map(|p| format!("#{}", p)).collect::<Vec<_>>().join(",");
    writeln!(out, "#{}=POLYLINE('top_outline',({}));", polyline_top, pts_list).unwrap();

    // Geometric representation context
    let geo_ctx = id; id += 1;
    writeln!(out, "#{}=GEOMETRIC_REPRESENTATION_CONTEXT(3);", geo_ctx).unwrap();

    let global_uc = id; id += 1;
    writeln!(out, "#{}=GLOBAL_UNCERTAINTY_ASSIGNED_CONTEXT(0.001);", global_uc).unwrap();

    let repr_ctx = id; id += 1;
    ...
    writeln!(out, "#{}=SHAPE_REPRESENTATION('Board',(#{},#{},#{}),#{});",
        shape_rep, axis, polyline_bottom, polyline_top, repr_ctx).unwrap();
