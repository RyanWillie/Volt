...
electrical, net-class, parts, BOM sourcing, assembly CPL option boundaries, KiCad adapter
boundaries, and the `Circuit` aggregate root throw typed kernel errors today; validation
...
subsystems (schematic, PCB outside the assembly CPL option boundary, IO, and authoring)
...
connectivity, hierarchy, electrical, net-class, parts, BOM sourcing, assembly CPL option,
and authoring boundaries throw typed kernel errors today; validation reports
...
PCB outside the assembly CPL option boundary, IO, adapters) still throw raw
