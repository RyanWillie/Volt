...
def _write_manufacturing_package(
    ...
    *,
    ...
    shutil.rmtree(manufacturing_root, ignore_errors=True)
    ...
    profile_payload = {
        "config": _manufacturing_profile_payload(config),
        "board": board_profile,
    }
...
def _native_fabrication_payload(native_export) -> dict[str, object]:
    warnings = [
        {
            "kind": warning.kind,
...
def _native_diagnostics_payload(diagnostics) -> dict[str, object]:
...
def _native_diagnostic_payload(diagnostic) -> dict[str, object]:
    return {
