...
[[nodiscard]] BoardFixture make_board_fixture() {
    ...
    const auto third_net =
        circuit.add_net(volt::NetSpec{volt::NetName{"C"}, volt::NetKind::Signal});
...
collect_board_shapes(const volt::Board &board) {
    const auto view = resolved(board);
    return volt::detail::collect_copper_shapes(view, volt::queries::resolve_pads(view));
}
...
validated_clearance_signatures(const volt::Board &board) {
    const auto report = volt::validate_board(resolved(board));
    auto signatures = std::vector<ClearanceDiagnosticSignature>{};
...
TEST_CASE("BoardSpatialIndex reports net-class pair clearance using the larger class value") {
    ...
    fixture.circuit.update(fixture.first_net,
                           volt::AssignNetClass{fixture.circuit.define_net_class(
                               volt::NetClassSpec{.net_class = std::move(first_class)})});
    auto second_class = volt::NetClass{volt::NetClassName{"B rules"}};
    second_class.set_copper_clearance_mm(0.50);
    fixture.circuit.update(fixture.second_net,
                           volt::AssignNetClass{fixture.circuit.define_net_class(
                               volt::NetClassSpec{.net_class = std::move(second_class)})});
...
TEST_CASE("BoardSpatialIndex lets a room override replace larger class and matrix clearances") {
    ...
    fixture.circuit.update(fixture.first_net,
                           volt::AssignNetClass{fixture.circuit.define_net_class(
                               volt::NetClassSpec{.net_class = std::move(net_class)})});
...
TEST_CASE("BoardSpatialIndex query output is deterministic across equivalent builds") {
    ...
    const auto first_index = volt::BoardSpatialIndex{resolved(board)};
    const auto second_index = volt::BoardSpatialIndex{resolved(board)};
