...
|---|---|---|
| **Use when** | Loose scripting, experiments, reusable sub-circuits | Product workflows — deterministic staged builds, full artifact bundles, project tests |
| **Creates** | One logical circuit owned by the kernel | Registered stages that author `Design`, `Schematic`, and `Board` models in order |
| **Output** | `design.write(path)` for logical JSON; `sch.write_json(path)` / `sch.write_svg(path)` for schematics | `result.write(path)` for a deterministic versioned bundle |
...
# Option 2: manufacturing handoff
...
| Intent | Skill |
|---|---|
| Define a reusable exact part (pins, symbol, footprint, identity) | `volt-component-authoring` |
...
If you are doing an end-to-end build from scratch, read `volt-component-authoring` first,
then `volt-circuit-authoring`, `volt-schematic-authoring`, and `volt-pcb-authoring`.
