...
pub fn autoplace_schematic(project: &Path, sch_name: &str) -> Result<()> {
    ...
    let comp_roles =
        analysis::classify_components(&circuit, &net_classes, &net_members, &lib_comps, &lib_syms);
    let flow_dag = analysis::build_flow_dag(
        &circuit,
        &net_classes,
        &net_members,
        &comp_roles,
    ...
    let companions = analysis::detect_companions(
        &circuit,
        &comp_roles,
        &net_classes,
        &net_members,
    ...
    let companion_set: std::collections::HashSet<Uuid> =
        companions.iter().map(|c| c.component).collect();

    // Phase 2: Ranking
    let main_components: Vec<Uuid> = circuit
        .components
        .iter()
        .map(|c| c.uuid)
    ...
    for inst in &circuit.components {
        let Some(pl) = placements.get(&inst.uuid) else {
            continue;
        };
        ...
        let Some(gate) = variant.gates.first() else {
            continue;
        };
        ...
        let texts =
            text_fields::build_symbol_texts(px, py, pl.rotation, lib_sym, &inst.name, &inst.value);
    ...
    let sym_by_comp: HashMap<Uuid, &SchematicSymbol> =
        schematic.symbols.iter().map(|s| (s.component, s)).collect();
    let comp_boxes = compute_component_boxes(&schematic, &circuit, &lib_comps, &lib_syms);

    for net in &circuit.nets {
        let net_class = net_classes
            .get(&net.uuid)
            .copied()
