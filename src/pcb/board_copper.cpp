...
void validate_board_drc(const Board &board, const FootprintLibrary &footprints,
    ...
    const auto footprint_geometries = board.project_footprint_geometries(footprints);
    ...
    rules
        ...
        .add([&footprint_geometries](const Board &, DiagnosticReport &rule_report) {
            validate_component_geometry_overlaps(footprint_geometries, rule_report);
        })
        .add([&shapes](const Board &rule_board, DiagnosticReport &rule_report) {
