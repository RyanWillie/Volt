...
TEST_CASE("Board rejects route endpoint intent with unresolved footprint data") {
    ...
    missing_mapping.circuit.select_physical_part(
...
TEST_CASE("Board derives ratsnest edges across bound module port nets") {
    ...
    auto board = volt::Board{fixture.circuit};
    const auto module_placement = board.place_component(volt::ComponentPlacement{
        fixture.module_component, volt::BoardPoint{10.0, 10.0}, volt::BoardRotation::degrees(0.0)});
    const auto parent_placement = board.place_component(volt::ComponentPlacement{
        fixture.parent_component, volt::BoardPoint{20.0, 10.0}, volt::BoardRotation::degrees(0.0)});

    const auto edges = board.ratsnest_edges(volt::builtin_footprint_library());

    REQUIRE(edges.size() == 1);
    CHECK(edges[0].net() == fixture.parent_net);
    CHECK(edges[0].from().placement() == module_placement);
    CHECK(edges[0].to().placement() == parent_placement);
}
