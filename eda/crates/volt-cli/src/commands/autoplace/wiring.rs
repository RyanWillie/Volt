...
use volt_core::project::{
    Circuit, Junction, LineEndpoint, Net, NetLabel,
    SchematicLine, SchematicNetSegment, SchematicSymbol,
};
...
pub fn route_net(
    ...
    match net_class {
        NetClass::Power | NetClass::HighFanout => {
            route_power_net(net, &endpoints)
        }
        NetClass::Signal => {
            route_signal_net(net, &endpoints, comp_boxes)
        }
    }
}
...
pub fn collect_pin_endpoints(
    ...
    for inst in &circuit.components {
        for sc in &inst.signal_connections {
            if sc.net != Some(net.uuid) { continue; }
            let Some(sym) = sym_by_comp.get(&inst.uuid) else { continue };
            ...
            let gate = variant.gates.iter().find(|g| g.uuid == sym.lib_gate)
            ...
            for pm in &gate.pin_mappings {
                if pm.signal != sc.signal { continue; }
                let Some(pin) = lib_sym.pins.iter().find(|p| p.uuid == pm.pin) else { continue };
...
fn route_pair(
    ...
    let from_ep = LineEndpoint::Symbol { symbol: sym_a, pin: pin_a };
    let to_ep = LineEndpoint::Symbol { symbol: sym_b, pin: pin_b };
    ...
    if dx < 0.01 || dy < 0.01 {
        if !segment_hits_obstacle(pos_a, pos_b, comp_boxes, &[sym_a, sym_b]) {
            lines.push(SchematicLine {
                uuid: new_uuid(), width: WIRE_WIDTH, from: from_ep, to: to_ep,
            });
            return;
        }
        ...
        junctions.push(Junction { uuid: j1_uuid, position: j1_pos });
        junctions.push(Junction { uuid: j2_uuid, position: j2_pos });
        lines.push(SchematicLine { uuid: new_uuid(), width: WIRE_WIDTH, from: from_ep, to: LineEndpoint::Junction { junction: j1_uuid } });
        lines.push(SchematicLine { uuid: new_uuid(), width: WIRE_WIDTH, from: LineEndpoint::Junction { junction: j1_uuid }, to: LineEndpoint::Junction { junction: j2_uuid } });
        lines.push(SchematicLine { uuid: new_uuid(), width: WIRE_WIDTH, from: LineEndpoint::Junction { junction: j2_uuid }, to: to_ep });
        return;
    }
    ...
    if hfirst_clear || vfirst_clear {
        ...
        let bend = if hfirst_clear { bend_hfirst } else { bend_vfirst };
        let junc_uuid = new_uuid();
        junctions.push(Junction { uuid: junc_uuid, position: bend });
        lines.push(SchematicLine { uuid: new_uuid(), width: WIRE_WIDTH, from: from_ep, to: LineEndpoint::Junction { junction: junc_uuid } });
        lines.push(SchematicLine { uuid: new_uuid(), width: WIRE_WIDTH, from: LineEndpoint::Junction { junction: junc_uuid }, to: to_ep });
        return;
    }
...
fn route_multi(
    ...
    sorted.sort_by(|&a, &b| {
        endpoints[a].2.y.partial_cmp(&endpoints[b].2.y)
    ...
    for &idx in &sorted {
        ...
        let junc_uuid = new_uuid();
        junctions.push(Junction { uuid: junc_uuid, position: trunk_pos });
        trunk_junctions.push((junc_uuid, trunk_pos));

        let from_ep = LineEndpoint::Symbol { symbol: sym, pin };
        let to_ep = LineEndpoint::Junction { junction: junc_uuid };
        ...
        if (pos.x - trunk_x).abs() < 0.01 {
            ...
            junctions.push(Junction { uuid: j_mid, position: j_mid_pos });

            let j_corner = new_uuid();
            let j_corner_pos = Position::new(snap_to_grid(trunk_x), snap_to_grid(clear_y));
            junctions.push(Junction { uuid: j_corner, position: j_corner_pos });

            // Pin → straight down/up → horizontal to trunk X → straight to trunk Y
            lines.push(SchematicLine { uuid: new_uuid(), width: WIRE_WIDTH, from: from_ep, to: LineEndpoint::Junction { junction: j_mid } });
            lines.push(SchematicLine { uuid: new_uuid(), width: WIRE_WIDTH, from: LineEndpoint::Junction { junction: j_mid }, to: LineEndpoint::Junction { junction: j_corner } });
            lines.push(SchematicLine { uuid: new_uuid(), width: WIRE_WIDTH, from: LineEndpoint::Junction { junction: j_corner }, to: to_ep });
        }
    }
    ...
    for pair in trunk_junctions.windows(2) {
        lines.push(SchematicLine {
            uuid: new_uuid(), width: WIRE_WIDTH,
            from: LineEndpoint::Junction { junction: pair[0].0 },
            to: LineEndpoint::Junction { junction: pair[1].0 },
        });
    }
}
...
fn segment_hits_obstacle(
    a: Position, b: Position, comp_boxes: &[BBox], _skip_syms: &[Uuid],
) -> bool {
...
fn find_clear_x_for_segment(
    a: Position, b: Position, comp_boxes: &[BBox], skip_syms: &[Uuid],
) -> f64 {
    let min_y = a.y.min(b.y);
    let max_y = a.y.max(b.y);
...
fn find_clear_trunk_x(
    ...
    let max_ep_x = endpoints.iter()
        .map(|e| e.2.x)
        .fold(f64::NEG_INFINITY, f64::max);
    let min_ep_x = endpoints.iter()
        .map(|e| e.2.x)
...
fn find_clear_y_for_hstub(
    pin_pos: Position, trunk_pos: Position, comp_boxes: &[BBox], _net_syms: &[Uuid],
) -> f64 {
    ...
    let blockers: Vec<&BBox> = comp_boxes.iter()
    ...
    let bottom_edge = blockers.iter().map(|b| b.y1).fold(f64::NEG_INFINITY, f64::max);
